        ...
        .then(users => {
            ...
            users.forEach(user => {
                ...
                if (user.role === 'student') {
                    loggedTime = `${user.hours}h ${user.minutes}m ${user.seconds}s`;
                }
                ...
                row.innerHTML = `
                    ...
                    <td>${loggedTime}</td>
                    <td><button onclick="viewLogs(${user.id})">Logs</button></td>
...
function viewLogs(userId) {
    const xhr = new XMLHttpRequest();
    xhr.open("POST", "php/get_user_logs.php", true);
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    xhr.onreadystatechange = function () {
        if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
            ...
            logs.forEach(log => {
                ...
                row.innerHTML = `
                    ...
                    <td>${log.end_time}</td>
                    <td>${log.duration_hours}h ${log.duration_minutes}m ${log.duration_seconds}s</td>
                `;
...
function showViewLogs() {
    document.getElementById('viewLogsForm').style.display = 'block';
}

function hideViewLogs() {
    document.getElementById('viewLogsForm').style.display = 'none';
    document.getElementById('logsTableBody').innerHTML = '';  // Clear the logs table
}
